    ...
    public function add() {
        ...
        if (!$this->ion_auth->is_admin()){
         ...
         redirect('/');
            ...
            $this->form_validation->set_rules('vendedor_email', '', 'trim|valid_email|max_length[50]|is_unique[vendedores.vendedor_email]');
    ...
    public function edit($vendedor_id = NULL) {
      ...
      if(!$vendedor_id || !$this->core_model->get_by_id('vendedores', array('vendedor_id' => $vendedor_id))){
          ...
          redirect('vendedores');
            ...
            $this->form_validation->set_rules('vendedor_email', '', 'trim|valid_email|max_length[50]|callback_check_vendedor_email');
...
//        exit();
    ...
    public function check_vendedor_rg($vendedor_rg) {
     ...
     if($this->core_model->get_by_id('vendedores', array('vendedor_rg' => $vendedor_rg,  'vendedor_id !=' => $vendedor_id))){
         $this->form_validation->set_message('check_vendedor_rg', 'RG já cadastrado');
         return FALSE; 
    ...
    public function check_vendedor_email($vendedor_email) {
     ...
     if($this->core_model->get_by_id('vendedores', array('vendedor_email' => $vendedor_email,  'vendedor_id !=' => $vendedor_id))){
         $this->form_validation->set_message('check_vendedor_email', 'Email já cadastrado');
         return FALSE; 
    ...
    public function check_vendedor_telefone($vendedor_telefone) {
     ...
     if($this->core_model->get_by_id('vendedores', array('vendedor_telefone' => $vendedor_telefone,  'vendedor_id !=' => $vendedor_id))){
         $this->form_validation->set_message('check_vendedor_telefone', 'Telefone já cadastrado');
         return FALSE; 
    ...
    public function check_vendedor_celular($vendedor_celular) {
     ...
     if($this->core_model->get_by_id('vendedores', array('vendedor_celular' => $vendedor_celular,  'vendedor_id !=' => $vendedor_id))){
         $this->form_validation->set_message('check_vendedor_celular', 'Celular já cadastrado');
         return FALSE; 
...
//                    $d += $cpf[$c] * (($t + 1) - $c); Para PHP com versão < 7.4
    ...
    public function preenche_endereco_vendedores(){
        ...
        if ($this->form_validation->run()) {
             ...
             */
            $cep = str_replace("-", "", $this->input->post('vendedor_cep'));
            ...
            if (isset($resultado_requisicao->erro)) {
                ...
                $retorno['erro'] = 3;
                $retorno['vendedor_cep'] = 'Informe um CEP válido';
                ...
                $retorno['erro'] = 0;
                $retorno['vendedor_endereco'] = $resultado_requisicao->logradouro;
                $retorno['vendedor_bairro'] = $resultado_requisicao->bairro;
                $retorno['vendedor_cidade'] = $resultado_requisicao->localidade;
                $retorno['vendedor_estado'] = $resultado_requisicao->uf;
             ...
             */
            $retorno['erro'] = 3;
            $retorno['vendedor_cep'] = form_error('vendedor_cep', '<div class="text-danger">', '</div>');
